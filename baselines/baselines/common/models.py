...
@register("lidar_cnn")
def lidar_cnn(**conv_kwargs):
    """
    My CNN for lidar observations.
    """
    def network_fn(input_shape):
        print('input shape is {}'.format(input_shape))
        x_input = tf.keras.Input(shape=input_shape)
        h = tf.expand_dims(x_input, -1)
        h = tf.math.divide(tf.math.add(h, 1), 2)
        with tf.name_scope("convnet"):
            h = tf.keras.Sequential(
                [tf.keras.layers.Conv2D(filters=32, kernel_size=3, strides=1, activation='relu', name='c1'),
                tf.keras.layers.MaxPool2D(2),
                tf.keras.layers.Conv2D(filters=64, kernel_size=3, strides=1, activation='relu', name='c2'),
                tf.keras.layers.MaxPool2D(2),
                tf.keras.layers.Conv2D(filters=64, kernel_size=3, strides=1, activation='relu', name='c3'),        
                tf.keras.layers.GlobalAvgPool2D(),
                tf.keras.layers.Dense(2, activation=None)])(h)
...
@register("world_model_mlp")
def mlp():
    """
    My MLP for world models
    """
    ...
    def network_fn(input_shape):
        ...
        h = tf.keras.Sequential(
            [
                tf.keras.layers.Dense(units=16, kernel_initializer=ortho_init(np.sqrt(2)), name='d1', activation='tanh'),
                tf.keras.layers.Dense(units=32, kernel_initializer=ortho_init(np.sqrt(2)), name='d2', activation='tanh'),
                tf.keras.layers.Dense(units=2, kernel_initializer=ortho_init(np.sqrt(2)), name='d3', activation=None)
            ]
        )(h)
        network = tf.keras.Model(inputs=[x_input], outputs=[h])
        return network

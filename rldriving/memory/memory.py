...
class Memory:
    ...
    def __init__(
            self,
            # Input_dim = 8 - VAE latent_dim + 1 - steering + 1 - velocity + 1 - acceleration + 2 - actions = 13
            input_dim=13, 
            output_dim=8,
            num_timesteps=999,
            batch_size=100,
    ...
    def get_weights(self):
        ...
        for name, model in self.models.items():
            ...
            mod_weights[name] = weights
        return mod_weights
    
    def set_weights(self, mod_weights):
        """Set model weights """
        for name, model in self.models.items():
            model.set_weights(mod_weights[name])
    
    def stop_training(self, stop_training: bool):
        for name, model in self.models.items():
